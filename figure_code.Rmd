...
ggplot(dev_data, aes(x = Age.Months, group = Individual, color = Individual, linetype = Individual))+
  stat_ecdf(geom = "step", aes(y=..y.. * max(dev_data$Individual == dev_data$Individual)), size = 1) +
  labs(x = "Age in Months", y = "Proportion of Skills Attained")+
  scale_y_continuous(breaks = seq(0, max(table(dev_data$Individual)), by = 1))+
  coord_cartesian(xlim= c(0,60))+
  theme_classic()+
  scale_x_continuous()+
  scale_color_manual(values = palette_development)+
  theme(axis.text.x = element_text(angle = 0, size = 12, vjust = 0.5, face = "bold"), axis.text.y = element_text(size = 14, face = "bold"), axis.title = element_text(size = 16, face = "bold"), legend.text = element_text(size = 12), legend.title = element_text(size = 16, face = "bold"))
...
jpeg(filename = "./figures/figure_3/Figure_3 opt 2.jpeg", width = 12, height = 14, units = "in", res = 600)
...
find_medications_with_categories <- function(data) {
  data <- data %>%
    group_by(patient_uuid) %>%
    arrange(patient_uuid, medication_age_days_firstDate) %>%
    mutate(medication_number = row_number()) %>%
    mutate(medication_category = case_when(
      medication_number == 1 ~ "Medication 1",
      medication_number == 2 ~ "Medication 2",
      medication_number == 3 ~ "Medication 3",
      medication_number == 4 ~ "Medication 4",
      medication_number >= 5 ~ "5th or later"
    )) %>%
    ungroup()
...
result <- find_medications_with_categories(Ciitizen_medication)
...
```{r Figure-4AB}
library(reshape2)
order <- c("Oxcarbazepine", "Lacosamide", "Phenytoin", "Valproate", "Lamotrigine", "Carbamazepine", "Rufinamide", "Eslicarbazepine", "Clonazepam", "Clobazam", "Phenobarbital", "Vigabatrin", "Felbamate", "Primidone", "Stiripentol", "Tiagabine", "Zonisamide", "Gabapentin", "Ethosuximide", "Levetiracetam", "Briveracetam", "ACTH", "Epidiolex/CBD", "Topiramate", "Prednisolone", "Perampanel")
...
df_heatmap <- result %>%
  filter(!is.na(medication)) %>%  # Drop NA values
  group_by(medication_category, medication) %>%
  summarise(count = n(), .groups = 'drop') %>%
  left_join(total_counts, by = "medication") %>%
  mutate(normalized_count = count / total_count)

Figure_4A <- ggplot(df_heatmap, aes(y = factor(medication, level = rev(order)), x = factor(medication_category, level = c("Medication 1", "Medication 2", "Medication 3", "Medication 4", "5th or later")), fill = normalized_count))+
  geom_tile(color = "white")+
  scale_fill_gradient(low = "#bacfec", high = "#a70234")+
  labs(x = "Medications", y = "Medication Order", fill = "Frequency")+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, hjust = 1), plot.background = element_rect("white"))
print(Figure_4A)


# Prepare data for the stacked bar plot, dropping NA values
df_long <- result %>%
  filter(!is.na(medication))  # Drop NA values

# Generate Stacked Bar Plot
Figure_4B <- df_long %>%
  count(medication, medication_category) %>%
  ggplot(aes(x = factor(medication, level = rev(order)), y = n, fill = factor(medication_category, level = rev(c("Medication 1", "Medication 2", "Medication 3", "Medication 4", "5th or later"))))) +
  geom_bar(stat = "identity") +
  labs(x = "Medications", y = "Frequency") +
  scale_fill_manual(name = "Medication Category", values = c("#a70234", "#ce8080", "#e4b8b4", "#f1b682","#ffde76")) +
  theme(axis.text.x = element_text(angle = 0, size = 12, vjust = 0.5, face = "bold"), axis.text.y = element_text(size = 14, face = "bold"), axis.title = element_text(size = 16, face = "bold"))+
...
ggsave(filename = "./figures/figure_4/Figure_4B hor.jpeg", height = 12, width = 16, dpi = 600, plot = Figure_4B)
```
...
classifier <- read_excel("./data/ciitizen_health_classifier.xlsx")
LOF_ids <- classifier$subgroup_lof
DEE_ids <- classifier$subgroup_dee
non_dee_ids <- classifier$subgroup_nondee
...
data_dee <- data_final %>% filter(patient_uuid %in% DEE_ids)
data_non_dee <- data_final %>% filter(patient_uuid %in% non_dee_ids)

average_medication_lof <- data_lof %>% 
  group_by(age_years) %>% 
  summarise(average_medication_count = mean(medication_count), .groups = "drop")

average_medication_dee <- data_dee %>% 
  group_by(age_years) %>% 
  summarise(average_medication_count = mean(medication_count), .groups = "drop")

average_medication_non_dee <- data_non_dee %>% 
  group_by(age_years) %>% 
  summarise(average_medication_count = mean(medication_count), .groups="drop")

Figure_4C <- ggplot()+
  geom_line(data = data_dee, aes(x= age_years, y = medication_count, group = patient_uuid), alpha = 0.5)+
  geom_smooth(data = average_medication_dee, aes(x = age_years, y = average_medication_count), 
...
Figure_4D <- ggplot()+
  geom_line(data = data_non_dee, aes(x = age_years, y = medication_count, group = patient_uuid), alpha = 0.5)+
  geom_smooth(data = average_medication_non_dee, aes(x = age_years, y = average_medication_count),
...
ggsave(filename = "./figures/figure_4/Figure_4E.jpeg", height = 12, width = 16, dpi = 600, plot = Figure_4E)
```
