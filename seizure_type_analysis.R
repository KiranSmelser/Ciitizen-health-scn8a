...
for (predictor in unique(results_df$predictor)) {
  ...
  # Determine n
  obs = c()
  for (seizure in predictor_df$seizure_type) {
    if (grepl("Onset_group", predictor)) {
      if (predictor == "Onset_group_(Intercept)") {
        subset_data <- data[data[["Onset_group"]] == "Onset_group_1-4 Months" & data[[seizure]] == 1, ]
        obs = append(obs, nrow(subset_data))
      } else {
        subset_data <- data[data[["Onset_group"]] == predictor & data[[seizure]] == 1, ]
        obs = append(obs, nrow(subset_data))
      }
    } else if (grepl("unique_types", predictor)) {
      if (predictor == "unique_types_(Intercept)") {
        subset_data <- data[data[["unique_types"]] == 0 & data[[seizure]] == 1, ]
        obs = append(obs, nrow(subset_data))
      } else {
        if (grepl("2+", predictor)) {
          subset_data <- data[data[["unique_types"]] == "2+" & data[[seizure]] == 1, ]
          obs = append(obs, nrow(subset_data))
        } else {
          subset_data <- data[data[["unique_types"]] == substring(predictor, nchar(predictor)) & data[[seizure]] == 1, ]
          obs = append(obs, nrow(subset_data))
        }
      }
    } else if (predictor == "age_onset_m") {
      subset_data <- data[data[[seizure]] == 1, ]
      obs = append(obs, nrow(subset_data))
    } else {
      subset_data <- data[data[[predictor]] == 1 & data[[seizure]] == 1, ]
      obs = append(obs, nrow(subset_data))
    }
  }
  predictor_df$n <- obs
